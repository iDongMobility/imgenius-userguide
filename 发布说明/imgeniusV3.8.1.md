...
添加下图的文件中部分，在EOC上就可以看到画面配置监控的功能。
  ...
  ![350](./3.8.1_res/images/22.png)

#### 配置说明

* 这里提供两个样例文件[画面组态与监控体验版样例(1)](发布说明/3.8.1_res/画面组态与监控体验版样例(1).txt)和[画面组态与监控体验版样例(2)](发布说明/3.8.1_res/画面组态与监控体验版样例(2).txt)，在画面配置中选择其他，编辑图，将文件内容粘贴进去选择替换现有画面即可。

  ![350](./3.8.1_res/images/23.png)
* 首先分别将两个画面保存，在文件中选择保存即可，然后点开画面1。

  ![350](./3.8.1_res/images/24.png)
...
* 在新建解决方案时，可选择内置模板，不选择则为空模板，空模板没有预设的例外等级、自定义列表和工程单位。而内置模板会有一些对应内容。
  ...
  ![350](./3.8.1_res/images/39.png)
* 详细可看[模板详细说明](http://userguide.idongmobility.cn/#/%E7%B3%BB%E7%BB%9F%E9%85%8D%E7%BD%AE%E6%89%8B%E5%86%8C/%E6%A8%A1%E6%9D%BF%E8%AF%A6%E7%BB%86%E8%AF%B4%E6%98%8E/%E6%A6%82%E8%BF%B0)

### PBIN662：作业组下多次引用同一个作业规范时，引用作业规范引用名自动命名为一个非重复引用名

* 在一个作业组下多次引用同一个作业规范时，如果这个作业规范有引用名，那么在作业组下每一次引用时都会自动重命名这个引用名，与之前的不会重复。

  ![350](./3.8.1_res/images/40.png)
